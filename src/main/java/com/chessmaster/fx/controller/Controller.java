    ...
    private void moverPecaVisualmente(Casa origem, Casa destino, boolean promocaoOcorreuLogica) {
        ...
        if (spDestino != null) {
            ...
            if (pecaVisual != null) {
                ...
                String caminhoRecurso = "/imagens/Peca" + nomePeca + corPeca + ".png";
                try(InputStream is = getClass().getResourceAsStream(caminhoRecurso)){
                    if(is != null){
        ...
        if(promocaoOcorreuLogica && destino.getPeca() instanceof Rainha){
            String caminhoImagemRainha = (destino.getPeca().getCor() == Cor.BRANCA)
                ? "/imagens/PecaRainhaBranca.png"
                : "/imagens/PecaRainhaPreta.png";
