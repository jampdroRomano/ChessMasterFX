...
public class Controller {
    ...
    @FXML
    private StackPane a1, b1, c1, d1, e1, f1, g1, h1;
    @FXML
    private StackPane a2, b2, c2, d2, e2, f2, g2, h2;
    @FXML
    private StackPane a3, b3, c3, d3, e3, f3, g3, h3;
    @FXML
    private StackPane a4, b4, c4, d4, e4, f4, g4, h4;
    @FXML
    private StackPane a5, b5, c5, d5, e5, f5, g5, h5;
    @FXML
    private StackPane a6, b6, c6, d6, e6, f6, g6, h6;
    @FXML
    private StackPane a7, b7, c7, d7, e7, f7, g7, h7;
    @FXML
    private StackPane a8, b8, c8, d8, e8, f8, g8, h8;
    ...
    private Bot bot;
    private boolean jogoContraBot = false;
    private boolean aguardandoBot = false;
    ...
    public void ativarJogoContraBot(Bot.NivelDificuldade nivel) {
        this.jogoContraBot = true;
        this.bot = new Bot(tabuleiro, Cor.PRETA, nivel);
        System.out.println("Jogo contra bot ativado - Nível: " + nivel);
    }
    ...
    private void onCasaClicked(StackPane spClicado) {
        if (jogoAcabou || aguardandoBot) {
            return;
        }
        
        if (jogoContraBot && turnoAtual == Cor.PRETA) {
            return;
        }
    ...
    private void executarMovimentoBot() {
        aguardandoBot = true;
        gridPaneTabuleiro.setDisable(true);
        
        PauseTransition delay = new PauseTransition(Duration.millis(800));
        delay.setOnFinished(event -> {
            Bot.Movimento movimentoBot = bot.calcularMelhorMovimento();
            
            if (movimentoBot != null) {
                Casa origem = movimentoBot.getOrigem();
                Casa destino = movimentoBot.getDestino();
                
                System.out.println("Bot move de " + 
                    obterNotacaoCasa(origem) + " para " + 
                    obterNotacaoCasa(destino));
            ...
            aguardandoBot = false;
            gridPaneTabuleiro.setDisable(false);
        });
        delay.play();
    }
    
    /**
     * Retorna a notação algébrica de uma casa (ex: e2, d4).
     */
    private String obterNotacaoCasa(Casa casa) {
        char coluna = (char) ('a' + casa.getColuna());
        int linha = 8 - casa.getLinha();
        return "" + coluna + linha;
    }
    ...
    private void verificarFimDeJogoAposMovimento(Peca pecaCapturada) {
        ...
        if (vencedor != null) {
            ...
            if (jogoContraBot) {
                String mensagem = vencedor.equals("BRANCAS") 
                    ? "Parabéns! Você venceu o bot!" 
                    : "O bot venceu! Tente novamente.";
                mostrarDialogoFimDeJogo(mensagem);
            } else {
                mostrarDialogoFimDeJogo("As " + vencedor + " venceram!");
            }
        }
    }

    private void mostrarDialogoFimDeJogo(String mensagem) {
